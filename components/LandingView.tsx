...
function CtaButton({
  ...
  if (a.type === 'page') {
    const href = a.page === 'quiz' ? '/quiz?preview=1' : a.page === 'results' ? '/results/preview' : basePath || '/';
    return <a href={href} className={className} {...rich(label)} />;
  }
  return <button data-start-scorecard className={className} {...rich(label)} />;
}

// The public landing page for one scorecard. Used at "/" (default scorecard)
// and at "/s/[id]" (each scorecard's own distinct URL).
export default function LandingView({
  ...
  const basePath = scorecardId != null ? `/s/${scorecardId}` : '';
